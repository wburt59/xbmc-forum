...
if($mybb->input['action'] == "findguest")
{
    $where_sql = "uid='0'";
...
elseif($mybb->input['action'] == "finduser")
{
    if ($mybb->input['uid'])
        ...
        $where_sql = "username='".$db->escape_string($mybb->input['username'])."'";
...
elseif($mybb->input['action'] == "finduserthreads")
{
    if ($mybb->input['uid'])
        ...
        $where_sql = "t.username='".$db->escape_string($mybb->input['username'])."'";
...
elseif($mybb->input['action'] == "getunread")
{
    if($mybb->user['uid'] == 0)
    {
    ...
    if ($db->type == "pgsql")
    {
        $query = $db->query("
            ...
            WHERE t.visible=1 AND t.closed NOT LIKE 'moved|%' AND t.lastpost > COALESCE(tr.dateline,$cutoff) AND t.lastpost > COALESCE(fr.dateline,$cutoff) AND t.lastpost>$cutoff {$where_sql}
        ");
    }
    else
    {
        $query = $db->query("
            ...
            LEFT JOIN ".TABLE_PREFIX."threadsread tr ON (tr.tid=t.tid AND tr.uid='{$mybb->user['uid']}')
            LEFT JOIN ".TABLE_PREFIX."forumsread fr ON (fr.fid=t.fid AND fr.uid='{$mybb->user['uid']}')
            WHERE t.visible=1 AND t.closed NOT LIKE 'moved|%' AND t.lastpost > IFNULL(tr.dateline,$cutoff) AND t.lastpost > IFNULL(fr.dateline,$cutoff) AND t.lastpost>$cutoff {$where_sql}
        ");
    }
    ...
    if (!$tids) error($lang->error_nosearchresults);
...
elseif($mybb->input['action'] == "do_search" && $mybb->request_method == "post")
{
    ...
    $search_data = array(
        ...
        "tid" => $mybb->input['tid'],   // tapatalk add
...
elseif($mybb->input['action'] == "getsubs")
{
    if($mybb->user['uid'] == 0)
    {
        $lang->load("usercp");
        error($lang->no_thread_subscriptions);
    }
...
if($mybb->input['action'] == "results")
{
    ...
    $perpage = intval($mybb->input['perpage']); // tapatalk add
    ...
    if($mybb->user['uid'] == 0)
    {
        ...
        // tapatalk add for compatibility
        if (function_exists('my_unserialize'))
    ...
    if($search['resulttype'] == "threads")
    {
        ...
        if($mybb->usergroup['issupermod'] == 1)
        {
            ...
            $p_unapproved_where = "p.visible >= 0";   // tapatalk add
        }
        ...
        else
        {
            ...
            $p_unapproved_where = 'p.visible=1';  // tapatalk add
        }
        ...
        $query = $db->query("
            SELECT t.*, u.avatar, p.displaystyle AS threadprefix, ts.sid as is_sub
            FROM ".TABLE_PREFIX."threads t
            LEFT JOIN ".TABLE_PREFIX."users u ON (u.uid=t.lastposteruid)
            LEFT JOIN ".TABLE_PREFIX."threadprefixes p ON (p.pid=t.prefix)
            LEFT JOIN ".TABLE_PREFIX."threadsubscriptions ts ON (t.tid=ts.tid AND ts.uid='{$mybb->user['uid']}')
        ...
        foreach($thread_cache as $thread)
        {
            ...
            if($thread['lastpost'] > $last_read && $last_read)
            {
                ...
                $unreadpost = 1;
                $thread['unread'] = true; // tapatalk add
            }
            else
            {
            ...
            $query = $db->query("
                ...
                LIMIT 1
            ");
            if ($lastpost = $db->fetch_array($query))
            {
                $lastpost['prev'] = process_short_content($lastpost['message']);
                $thread['lastpost'] = $lastpost;
            }
            
            $thread['forumname'] = $forumcache[$thread['fid']]['name'];
            $results[] = $thread;
    ...
    else // Displaying results as posts
    {
        ...
        while($post = $db->fetch_array($query))
        {
            ...
            // tapatalk add
            $post['forumname'] = $forumcache[$post['fid']]['name'];
            $post['prev'] = $prev;
            $results[] = $post;
