	...
	initialize: function(textarea, options)
	{
		...
		$$('script').each(function(script) {
			if(script.src && script.src.indexOf('editor.js') != -1) {
				this.baseURL = script.src.replace(/xbmc\/js\/editor\.js(.*?)$/, 'jscripts/');
			}
		}, this);
...
*/
		...
		textInput.style.height = '250px';//parseInt(areaContainer.style.height)+"px";
		textInput.style.width = '99%';//parseInt(areaContainer.style.width)+"px";
