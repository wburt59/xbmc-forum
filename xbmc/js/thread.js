var Thread = {
	init: function()
	{
		$$('a.link-postReply').each(function(t) {
			t.observe('click', function(e) {
				if($('message').value.length) {
					$('quick_reply_preview').click();
					return e.preventDefault();
				}
			})
		});

		$$('div.post-body').each(function(t) {
			MyBB.enrichContent(t);
		});
	...
	initMultiQuote: function()
	{
		...
		if(quoted)
		{
			...
			post_ids.each(function(post_id) {
				if($("multiquote_link_"+post_id))
				{
					element = $("multiquote_link_"+post_id);
					element.addClassName('multiquoteSelected');
				}
			});
	...
	multiQuote: function(pid)
	{
		...
		{
			element.removeClassName('multiquoteSelected');
		}
		if($('quickreply_multiquote'))
		{
	...
	quickReplyDone: function(request)
	{
		...
		else if(request.responseText.match(/id="post_([0-9]+)"/))
		{
			...
			// add custom parsers
			MyBB.enrichContent($("pid_"+pid));
			// make quick edit possible on quick replied content
			if(use_xmlhttprequest == "1")
			{
